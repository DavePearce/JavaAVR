...
import static javaavr.core.AvrInstruction.Opcode.*;
import static javaavr.core.AvrInstruction.*;
import static javaavr.core.AVR.Memory;
...
public class AvrDecoder implements AVR.Decoder {
	...
	private static AvrInstruction decode_92(int opcode, Memory mem, int pc) {
			...
			int msb = mem.read(pc+3) & 0xFF;
			opcode = (msb << 24) | (lsb << 16) | opcode;
			int k = extract_s1111100011111111111111111(opcode);
			return new JMP(k);
	}
	...
	private static int extract_s1111100011111111111111111(int opcode) {
		int k = (opcode & 0b11111111111111111) >> 0;
		k |= (opcode & 0b1111100000000000000000000) >> 3;
		k = (k << 10) >> 10;
		return k;
	}
	private static int extract_u1111100000000000000000000(int opcode) {
		int Rd = (opcode & 0b1111100000000000000000000) >> 20;
		return Rd;
	}
	private static int extract_u1111111111111111(int opcode) {
		int k = (opcode & 0b1111111111111111) >> 0;
		return k;
	}
	...
	private static int extract_u11100001111(int opcode) {
		int k = (opcode & 0b0000000000001111) >> 0;
		k |= (opcode & 0b0000011100000000) >> 4;
		return k;
	}
}