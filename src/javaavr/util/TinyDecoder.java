...
public class TinyDecoder implements Instruction.Decoder {
	...
	private static Instruction decode_0(int opcode) {
	...
	private static Instruction decode_1(int opcode) {
			return new CPI(extract_0011_KKKK_dddd_KKKK(opcode));
	}
	private static Instruction decode_2(int opcode) {
	...
	private static Instruction decode_3(int opcode) {
	...
	private static Instruction decode_4(int opcode) {
			return new NOP(extract_0000_0000_0000_0000(opcode));
	}
	private static Instruction decode_5(int opcode) {
			return new MOVW(extract_0000_0001_dddd_rrrr(opcode));
	}
	private static Instruction decode_6(int opcode) {
			return new MULS(extract_0000_0010_dddd_rrrr(opcode));
	}
	private static Instruction decode_7(int opcode) {
	...
	private static Instruction decode_8(int opcode) {
			return new MULSU(extract_0000_0011_0ddd_0rrr(opcode));
	}
	private static Instruction decode_9(int opcode) {
			return new FMULS(extract_0000_0011_1ddd_0rrr(opcode));
	}
	private static Instruction decode_10(int opcode) {
			return new FMUL(extract_0000_0011_0ddd_1rrr(opcode));
	}
	private static Instruction decode_11(int opcode) {
			return new FMULSU(extract_0000_0011_1ddd_1rrr(opcode));
	}
	private static Instruction decode_12(int opcode) {
			return new CPC(extract_0000_01rd_dddd_rrrr(opcode));
	}
	private static Instruction decode_13(int opcode) {
			return new SBC(extract_0000_10rd_dddd_rrrr(opcode));
	}
	private static Instruction decode_14(int opcode) {
			return new ADD(extract_0000_11rd_dddd_rrrr(opcode));
	}
	private static Instruction decode_15(int opcode) {
		switch(opcode & 0b1111110000000000) {
		case 0b0001000000000000:
	...
	private static Instruction decode_16(int opcode) {
			return new CPSE(extract_0001_00rd_dddd_rrrr(opcode));
	}
	private static Instruction decode_17(int opcode) {
			return new CP(extract_0001_01rd_dddd_rrrr(opcode));
	}
	private static Instruction decode_18(int opcode) {
			return new SUB(extract_0001_10rd_dddd_rrrr(opcode));
	}
	private static Instruction decode_19(int opcode) {
			return new ADC(extract_0001_11rd_dddd_rrrr(opcode));
	}
	private static Instruction decode_20(int opcode) {
		switch(opcode & 0b1111110000000000) {
		case 0b0010000000000000:
	...
	private static Instruction decode_21(int opcode) {
			return new AND(extract_0010_00rd_dddd_rrrr(opcode));
	}
	private static Instruction decode_22(int opcode) {
			return new EOR(extract_0010_01rd_dddd_rrrr(opcode));
	}
	private static Instruction decode_23(int opcode) {
			return new OR(extract_0010_10rd_dddd_rrrr(opcode));
	}
	private static Instruction decode_24(int opcode) {
			return new MOV(extract_0010_11rd_dddd_rrrr(opcode));
	}
	private static Instruction decode_25(int opcode) {
			return new SBCI(extract_0100_KKKK_dddd_KKKK(opcode));
	}
	private static Instruction decode_26(int opcode) {
			return new SUBI(extract_0101_KKKK_dddd_KKKK(opcode));
	}
	private static Instruction decode_27(int opcode) {
			return new ORI(extract_0110_KKKK_dddd_KKKK(opcode));
	}
	private static Instruction decode_28(int opcode) {
			return new ANDI(extract_0111_KKKK_dddd_KKKK(opcode));
	}
	private static Instruction decode_29(int opcode) {
	...
	private static Instruction decode_30(int opcode) {
			return new LD_Z(extract_1000_000d_dddd_0000(opcode));
	}
	private static Instruction decode_31(int opcode) {
			return new ST_Z(extract_1000_001r_rrrr_0000(opcode));
	}
	private static Instruction decode_32(int opcode) {
			return new LD_Y(extract_1000_000d_dddd_1000(opcode));
	}
	private static Instruction decode_33(int opcode) {
			return new ST_Y(extract_1000_001r_rrrr_1000(opcode));
	}
	private static Instruction decode_34(int opcode) {
		switch(opcode & 0b1111110000000000) {
		case 0b1001000000000000:
	...
	private static Instruction decode_35(int opcode) {
		switch(opcode & 0b1111111000001111) {
	...
	private static Instruction decode_59(int opcode) {
		switch(opcode & 0b1111111000001111) {
		case 0b1001010000000000:
	...
	private static Instruction decode_60(int opcode) {
			return new COM(extract_1001_010d_dddd_0000(opcode));
	}
	private static Instruction decode_61(int opcode) {
			return new NEG(extract_1001_010d_dddd_0001(opcode));
	}
	private static Instruction decode_62(int opcode) {
			return new SWAP(extract_1001_010d_dddd_0010(opcode));
	}
	private static Instruction decode_63(int opcode) {
			return new INC(extract_1001_010d_dddd_0011(opcode));
	}
	private static Instruction decode_64(int opcode) {
			return new ASR(extract_1001_010d_dddd_0101(opcode));
	}
	private static Instruction decode_65(int opcode) {
			return new LSR(extract_1001_010d_dddd_0110(opcode));
	}
	private static Instruction decode_66(int opcode) {
			return new ROR(extract_1001_010d_dddd_0111(opcode));
	}
	private static Instruction decode_67(int opcode) {
	...
	private static Instruction decode_68(int opcode) {
			return new BSET(extract_1001_0100_0sss_1000(opcode));
	}
	private static Instruction decode_69(int opcode) {
			return new BCLR(extract_1001_0100_1sss_1000(opcode));
	}
	private static Instruction decode_70(int opcode) {
	...
	private static Instruction decode_71(int opcode) {
			return new RET(extract_1001_0101_0000_1000(opcode));
	}
	private static Instruction decode_72(int opcode) {
			return new RETI(extract_1001_0101_0001_1000(opcode));
	}
	private static Instruction decode_73(int opcode) {
		switch(opcode & 0b1111111111111111) {
		case 0b1001010110001000:
	...
	private static Instruction decode_74(int opcode) {
			return new SLEEP(extract_1001_0101_1000_1000(opcode));
	}
	private static Instruction decode_75(int opcode) {
			return new BREAK(extract_1001_0101_1001_1000(opcode));
	}
	private static Instruction decode_76(int opcode) {
			return new WDR(extract_1001_0101_1010_1000(opcode));
	}
	private static Instruction decode_77(int opcode) {
			return new LPM(extract_1001_0101_1100_1000(opcode));
	}
	private static Instruction decode_78(int opcode) {
			return new ELPM(extract_1001_0101_1101_1000(opcode));
	}
	private static Instruction decode_79(int opcode) {
			return new SPM(extract_1001_0101_1110_1000(opcode));
	}
	private static Instruction decode_80(int opcode) {
		switch(opcode & 0b1111111111111111) {
		case 0b1001010000001001:
	...
	private static Instruction decode_81(int opcode) {
			return new IJMP(extract_1001_0100_0000_1001(opcode));
	}
	private static Instruction decode_82(int opcode) {
			return new EIJMP(extract_1001_0100_0001_1001(opcode));
	}
	private static Instruction decode_83(int opcode) {
			return new ICALL(extract_1001_0101_0000_1001(opcode));
	}
	private static Instruction decode_84(int opcode) {
			return new EICALL(extract_1001_0101_0001_1001(opcode));
	}
	private static Instruction decode_85(int opcode) {
			return new DEC(extract_1001_010d_dddd_1010(opcode));
	}
	private static Instruction decode_86(int opcode) {
		switch(opcode & 0b1111111100000000) {
		case 0b1001011000000000:
	...
	private static Instruction decode_87(int opcode) {
			return new ADIW(extract_1001_0110_KKdd_KKKK(opcode));
	}
	private static Instruction decode_88(int opcode) {
			return new SBIW(extract_1001_0111_KKdd_KKKK(opcode));
	}
	private static Instruction decode_89(int opcode) {
		switch(opcode & 0b1111111100000000) {
		case 0b1001100000000000:
	...
	private static Instruction decode_90(int opcode) {
			return new CBI(extract_1001_1000_AAAA_Abbb(opcode));
	}
	private static Instruction decode_91(int opcode) {
			return new SBIC(extract_1001_1001_AAAA_Abbb(opcode));
	}
	private static Instruction decode_92(int opcode) {
			return new SBI(extract_1001_1010_AAAA_Abbb(opcode));
	}
	private static Instruction decode_93(int opcode) {
			return new SBIS(extract_1001_1011_AAAA_Abbb(opcode));
	}
	private static Instruction decode_94(int opcode) {
			return new MUL(extract_1001_11rd_dddd_rrrr(opcode));
	}
	private static Instruction decode_95(int opcode) {
	...
	private static Instruction decode_96(int opcode) {
			return new LDS(extract_1010_0kkk_dddd_kkkk(opcode));
	}
	private static Instruction decode_97(int opcode) {
			return new STS_DATA(extract_1010_1kkk_dddd_kkkk(opcode));
	}
	private static Instruction decode_98(int opcode) {
		switch(opcode & 0b1111100000000000) {
		case 0b1011000000000000:
	...
	private static Instruction decode_99(int opcode) {
			return new IN(extract_1011_0AAd_dddd_AAAA(opcode));
	}
	private static Instruction decode_100(int opcode) {
			return new OUT(extract_1011_1AAr_rrrr_AAAA(opcode));
	}
	private static Instruction decode_101(int opcode) {
			return new RJMP(extract_1100_kkkk_kkkk_kkkk(opcode));
	}
	private static Instruction decode_102(int opcode) {
			return new RCALL(extract_1101_kkkk_kkkk_kkkk(opcode));
	}
	private static Instruction decode_103(int opcode) {
			return new LDI(extract_1110_KKKK_dddd_KKKK(opcode));
	}
	private static Instruction decode_104(int opcode) {
		switch(opcode & 0b1111110000000000) {
		case 0b1111000000000000:
	...
	private static Instruction decode_105(int opcode) {
			return new BRBS(extract_1111_00kk_kkkk_ksss(opcode));
	}
	private static Instruction decode_106(int opcode) {
			return new BRBC(extract_1111_01kk_kkkk_ksss(opcode));
	}
	private static Instruction decode_107(int opcode) {
	...
	private static Instruction decode_108(int opcode) {
			return new BLD(extract_1111_100d_dddd_0bbb(opcode));
	}
	private static Instruction decode_109(int opcode) {
			return new BST(extract_1111_101d_dddd_0bbb(opcode));
	}
	private static Instruction decode_110(int opcode) {
		switch(opcode & 0b1111111000001000) {
		case 0b1111110000000000:
	...
	private static Instruction decode_111(int opcode) {
			return new SBRC(extract_1111_110r_rrrr_0bbb(opcode));
	}
	private static Instruction decode_112(int opcode) {
